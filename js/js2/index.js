...
$(window).load(function() {
    $('.banner').flexslider({
        ...
        before: function(slider) {
            ...
            TweenMax.fromTo(target.find('.text'), 2, {
                left: '95%'
            }, {
                left: '50%'
            });
            TweenMax.fromTo(target.find('.para'), 1.6, {
                left: '30%'
            }, {
                left: '0'
            });
    ...
    var curIndex = 0;
    var arr = ["#90c31f", '#33aa8f', "#1a974b", "#e99620", "#e54e6c", "#e56d4c", "#8c67da"];
    var tarNum = 0;
    kicks.bind('mouseenter', function() {
        var tarIndex = kicks.index(this);
        if (curIndex != tarIndex) {
            tarNum = tarIndex;
            TweenMax.to(kicks.eq(curIndex), 0.5, {
                width: 110
            });
            TweenMax.to(kicks.eq(curIndex).find('.icon'), 0.3, {
                backgroundColor: 'transparent'
            });
            TweenMax.to(kicks.eq(tarIndex), 0.5, {
                width: 535
            });
            TweenMax.to(kicks.eq(tarIndex).find('.icon'), 0.3, {
                backgroundColor: arr[tarNum]
            });
            curIndex = tarIndex;
        }
    });
